...
module.exports.create = (server, req, res, next) => {
    ...
    req.body.updateDate = moment().format('YYYY-MM-DD HH:mm');
...
module.exports.saveBulk = (server, req, res, next) => {
    ...
    for (let i = 0; i < contacts.length; i++) {
        ...
        if (contacts[i].id > 0) {
            server.api.models.contact.update(contacts[i]);
        } else {
            server.api.models.contact.create(contacts[i]);
        }
    }
